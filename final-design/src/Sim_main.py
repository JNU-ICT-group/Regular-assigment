...
commands = {
    ...
    'channel': os.path.join(python_executable, 'lib','IC', 'byteChannel.py'),
    'DMS_create': os.path.join(python_executable, 'lib','IS', 'generate.py'),
    'codec_source': os.path.join(python_executable, 'lib','IS_enorde', 'byteSourceCoder.py'),
    'codec_channel': os.path.join(python_executable, 'lib','IC_enorde', 'repetitionCoder.py'),
    'calc_source': os.path.join(python_executable, 'lib', 'calcDMSInfo.py'),
    'calc_channel': os.path.join(python_executable, 'lib', 'calcBSCInfo.py'),
    'calc_codec_source': os.path.join(python_executable, 'lib', 'calcCodecInfo.py'),
    'calc_codec_channel': os.path.join(python_executable, 'lib', 'calcErrorRate.py'),
    'calc_theory': os.path.join(python_executable, 'lib', 'calcInfo.py'),
}

# 使用命令字典
cmd_source = f'{commands["source"]} '
cmd_channel = f'{commands["channel"]} '
cmd_DMS_create = f'{commands["DMS_create"]} '
cmd_codec_source = f'{commands["codec_source"]} '
cmd_codec_channel = f'{commands["codec_channel"]} '
cmd_calc_source = f'{commands["calc_source"]} '
cmd_calc_channel = f'{commands["calc_channel"]} '
cmd_calc_codec_source = f'{commands["calc_codec_source"]} '
cmd_calc_codec_channel = f'{commands["calc_codec_channel"]} '
cmd_calc_theory = f'{commands["calc_theory"]} '
...
for i,case in enumerate(cases):
    ...
    """"
    信源-》信源编码-》信道编码-》信道-》信道解码-》信源解码-》实测指标-》理论计算和表格统计
    """
    ...
    # 计算实测指标
    source_pmf_path = os.path.join(data_dir, 'DMS.pmf.p0=%.3f.p=%.3f.csv' % (case['prob0'], error_rate))
