...
class PrefixCodec:
    ...
    def encode_streaming(self, data):
        ...
        if size > 0:
            ...
            buffer = (buffer << b) + v
            size += b
            # Cut off EOF, no need it for a file than can tell its end, just for avoid indistinguishable misreading.
            if size >= 8:
                byte = buffer >> (size - 8)
            else:
                byte = buffer << (8 - size)
...
class HuffmanCodec(PrefixCodec):
    ...
    def from_frequencies(cls, frequencies, concat=None, eof=_EOF):
        ...
        if eof not in frequencies:
            ...
            # make EOF has the minimum probability to "occur", so it will be assigned with the longest codeword,
            # witch not able to impact other codes.
            heap.append((0., [(eof, (0, 0))]))
